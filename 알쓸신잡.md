...
## 4. map
* map은 각 노드가 key와 value가 한 쌍으로 이루어진 트리로 중복이 허용되지 않는 것이 특징입니다.
* map의 정렬은 기본적으로 key를 기준으로 오른차순으로 정렬하며 내림차순을 사용해야하는 경우는 다음과 같습니다.	
	
	` map<int,int,greater> m; `
	
### 1) 헤더 및 map 선언
* 헤더: ` #include <map> `
* 선언: map<key type, value type> 이름 ` map<int,int> m; `
